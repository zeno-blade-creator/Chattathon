...
export async function generatePlan(corpus, intake, opts = {}) {
  ...
  const city = checkCity(normalised.city);
  ...
  const live = opts.live === false ? [] : await discoverLocal(profile);
  ...
  const actions = (data.week_one?.actions ?? []).slice(0, 5).map((a, i) => ({
    ...
    itemIds: (a.itemIds ?? []).filter(id => liveIds.has(id)),
