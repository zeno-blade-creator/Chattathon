...
// How long we wait for Supabase before using the bundled corpus. Deliberately
// short: on stage a slow fallback is indistinguishable from a broken demo, and
// the local copy is the same data.
const TIMEOUT_MS = Number(process.env.SUPABASE_TIMEOUT_MS ?? 2000);

// supabase-js retries internally and does not honour a per-request abort, so we
// bound the whole attempt ourselves. Measured: without this, an unreachable
// Supabase costs ~7s before the fallback — a dead pause in a live demo.
