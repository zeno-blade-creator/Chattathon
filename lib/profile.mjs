...
// There is a Boston in Lincolnshire, a Cambridge in England, a Bostonia in
// Ecuador and a Somerville in New Jersey. A substring match on "boston" sends
// all of them a Massachusetts plan, which is exactly the hallucinated-local-
// detail failure we cannot afford. So: word-boundary matching, then a check for
// any geography that rules Massachusetts out.
const DISQUALIFIERS = [
  // countries and regions that carry a same-named city
  ...
  // US states other than Massachusetts that have a Boston, Cambridge or Somerville
  ...
  'new hampshire', 'vermont', 'maine', 'connecticut', 'rhode island',
  // bare state abbreviations, matched with word boundaries
  'ny', 'nj', 'nh', 'vt', 'ct', 'ri', 'me', 'pa', 'oh', 'ga', 'tx', 'ca', 'fl',
];

// Explicitly re-qualifying: if they said Massachusetts, believe them over a
// stray token like "ME" that happened to appear.
 ...
 * @returns {{supported: boolean, matched: string|null, reason: string|null}}
 */
export function checkCity(input = '') {
  const s = String(input).toLowerCase().trim();
  if (!s) return { supported: false, matched: null, reason: 'no city given' };

  const matched = SUPPORTED_CITY_TERMS.find(t => wordRe(t).test(s)) ?? null;
  if (!matched) return { supported: false, matched: null, reason: 'outside Greater Boston' };

  const saidMass = QUALIFIERS.some(q => s.includes(q));
  const blocker = saidMass ? null : DISQUALIFIERS.find(d => wordRe(d).test(s));
  if (blocker) {
    return { supported: false, matched: null,
             reason: `looks like ${matched} in ${blocker}, not Massachusetts` };
  }
  return { supported: true, matched, reason: null };
}
...
export function buildProfile(raw) {
  ...
  const { supported, matched, reason } = checkCity(`${raw.city} ${raw.neighbourhood ?? ''}`);
  const profile = {
...
/** Convenience boolean wrapper around checkCity(). */
export function isSupportedCity(input = '') {
  return checkCity(input).supported;
}
